...
See [Preliminaries](preliminaries.mdx) for information on setting up one's environment for using this tutorial.
...
import SyncStatus from '../_partials/_check-sync-status.mdx';
...
Check that an address and key has been created for the seller. If not, see "Creating Addresses and Signing Keys" in [Preliminaries](preliminaries.mdx).
...
Check that an address and key has been created for the buyer. If not, see "Creating Addresses and Signing Keys" in [Preliminaries](preliminaries.mdx).
...
Check that an address and key has been created for the mediator. If not, see "Creating Addresses and Signing Keys" in [Preliminaries](preliminaries.mdx).
...
10. Select "Download as JSON", set the file name to "escrow-contract.json", and store the file in this folder. Note that most interfaces for Jupyter notebooks allow one to drag and drop a file into their explorer panel.
...
See [Lesson 1](runtime-cli.mdx) for an example of performing step 6.
